...
function RootLayout() {
  ...
  const recentMainRuns = useMemo(() => runs.filter((run) => run.pr == null).slice(0, 3), [runs])
  const recentPRRuns = useMemo(() => runs.filter((run) => run.pr != null).slice(0, 3), [runs])
  const failedMainCount = useMemo(
    () => runs.filter((run) => run.pr == null && ['failed', 'error'].includes(runStatus(run))).length,
    [runs],
  )
  const failedPRCount = useMemo(
    () => runs.filter((run) => run.pr != null && ['failed', 'error'].includes(runStatus(run))).length,
    [runs],
  )
    ...
    <div className="flex min-h-screen bg-white dark:bg-gray-950">
      ...
      >
        ...
        <div className="px-4 pb-3">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Recent Main</h3>
          <ul className="space-y-0.5">
            {recentMainRuns.map((run) => (
        ...
        <div className="px-4 pb-3">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Recent PR</h3>
          <ul className="space-y-0.5">
            {recentPRRuns.map((run) => (
              <li key={`${run.run_id}-${run.timestamp}`}>
                <Link
                  to="/run"
                  search={{ run: run.run_id }}
        ...
        <div className="px-4 pb-4">
          ...
          <Link
            ...
            search={{ group: 'failed' }}
            onClick={() => setMobileSidebarOpen(false)}
            className="mb-1.5 flex items-center justify-between rounded px-2 py-1.5 text-sm text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
          >
            <span>Main failed / error</span>
            <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">{failedMainCount}</span>
          </Link>
          <Link
            to="/pr"
            search={{ group: 'failed' }}
            onClick={() => setMobileSidebarOpen(false)}
            className="flex items-center justify-between rounded px-2 py-1.5 text-sm text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
          >
            <span>PR failed / error</span>
            <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">{failedPRCount}</span>
          </Link>
        </div>
      </aside>
